...
public sealed class SourcePresentationCacheTests
{
    ...
    public async Task Reuses_models_and_token_batches()
    {
        ...
        var first = await cache.GetLinesAsync(model, 0, 1, null, null, null, null, default);
        var second = await cache.GetLinesAsync(model, 0, 1, null, null, null, null, default);
    ...
    public async Task Assembly_removal_releases_its_models_and_batches()
    {
        ...
        var key = Key(1);
        var model = await cache.GetModelAsync(key, "class C {}");
        await cache.GetLinesAsync(model, 0, 1, null, null, null, null, default);
