...
public sealed class SourcePresentationCache(ILogger<SourcePresentationCache> logger)
{
    ...
    private readonly object noSpans = new();
    ...
    public async Task<IReadOnlyList<SourceTokenizedLine>> GetLinesAsync(SourceDocumentModel model, int start, int count,
        IReadOnlyDictionary<string, SymbolId?>? links, IReadOnlyDictionary<string, string>? typeKinds, IReadOnlyList<ReferenceSpan>? references,
        IReadOnlyList<ClassifiedSpan>? semanticSpans, CancellationToken token)
    {
        token.ThrowIfCancellationRequested();
        var key = new BatchKey(model.Key, start, count, links ?? noLinks, typeKinds ?? noKinds, references ?? noReferences, semanticSpans ?? noSpans);
        ...
        var lines = await model.TokenizeLinesAsync(start, count, links, typeKinds, references, semanticSpans, linked.Token);
        timer.Stop();
        token.ThrowIfCancellationRequested();
    ...
    private readonly record struct BatchKey(SourceDocumentKey Document, int Start, int Count, object Links, object TypeKinds, object References, object SemanticSpans);
